    ...
    stages {
        stage("Build"){
             agent {
                kubernetes {
                    inheritFrom 'default'
                    yamlFile 'ci/pod/diskAnn-build.yaml'
        ...
        stage("Test"){
            ...
            agent {
                kubernetes {
                    inheritFrom 'default'
                    yamlFile 'ci/pod/diskAnn-e2e.yaml'
                    defaultContainer 'main'
                }
            }
            steps {
                script{
                    ...
                    dir('tests'){
                      ...
                      sh "pip3 install ${knowhere_wheel} \
                          && pip3 install -r requirements.txt --timeout 30 --retries 6 &&  pytest --version"
                    }

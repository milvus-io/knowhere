...
class GraphRepository : public Repository<GRAPH_NODE> {
 ...
 public:
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
    void
    open(const std::string& file, size_t sharedMemorySize) {
    ...
    void
    insert(ObjectID id, ObjectDistances& objects) {
    ...
    inline GRAPH_NODE*
    get(ObjectID fid, size_t& minsize) {
...
class ReadOnlyGraphNode : public std::vector<std::pair<uint64_t, PersistentObject*>> {
 public:
    ...
    void
    push_back(std::pair<uint32_t, PersistentObject*> node) {
        (*this)[usedSize] = node;
        usedSize++;
    }
    size_t
    ...
    virtual int64_t
    memSize() {
        return reservedSize * (sizeof(uint64_t) + (*this)[0].second->memSize());
    }
    size_t reservedSize;
    size_t usedSize;
};
...
class SearchGraphRepository : public std::vector<ReadOnlyGraphNode> {
 public:
    ...
    void
    deserialize(std::ifstream& is, ObjectRepository& objectRepository) {
        if (!is.is_open()) {
 ...
 public:
    ...
    class Search {
     public:
        static void (*getMethod(NGT::ObjectSpace::DistanceType dtype, NGT::ObjectSpace::ObjectType otype,
                                size_t size))(NGT::NeighborhoodGraph&, NGT::SearchContainer&, NGT::ObjectDistances&) {
            if (size < 5000000) {
                switch (otype) {
                    default:
        ...
        static void
        l1Uint8(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        l2Uint8(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        l1Float(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        l2Float(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        hammingUint8(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        jaccardUint8(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        sparseJaccardFloat(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        cosineSimilarityFloat(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        angleFloat(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        ...
        static void
        sparseJaccardFloatForLargeDataset(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
        static void
        cosineSimilarityFloatForLargeDataset(NeighborhoodGraph& graph, NGT::SearchContainer& sc,
        ...
        normalizedCosineSimilarityFloatForLargeDataset(NeighborhoodGraph& graph, NGT::SearchContainer& sc,
                                                       ObjectDistances& seeds);
        static void
        normalizedAngleFloatForLargeDataset(NeighborhoodGraph& graph, NGT::SearchContainer& sc, ObjectDistances& seeds);
    };
#endif
    ...
    class Property {
     public:
        ...
        void
        set(NGT::Property& prop);
        void
        get(NGT::Property& prop);

        void
        exportProperty(NGT::PropertySet& p) {
            ...
            switch (graphType) {
                ...
                case NeighborhoodGraph::GraphTypeIANNG:
                    ...
                    break;
                default:
                    std::cerr << "Graph::exportProperty: Fatal error! Invalid Graph Type." << std::endl;
                    abort();
            }
            switch (seedType) {
                ...
                case NeighborhoodGraph::SeedTypeAllLeafNodes:
                    ...
                    break;
                default:
                    std::cerr << "Graph::exportProperty: Fatal error! Invalid Seed Type." << std::endl;
                    abort();
            }
        }
        void
        importProperty(NGT::PropertySet& p) {
            setDefault();
            ...
            if (it != p.end()) {
                if (it->second == "KNNG")
                    graphType = NeighborhoodGraph::GraphTypeKNNG;
            ...
            it = p.find("SeedType");
            if (it != p.end()) {
                if (it->second == "RandomNodes")
                    seedType = NeighborhoodGraph::SeedTypeRandomNodes;
    ...
    getNode(ObjectID fid, size_t& minsize) {
        return repository.get(fid, minsize);
    }
    inline GraphNode*
    getNode(ObjectID fid) {
        return repository.VECTOR::get(fid);
    }
    void
    insertNode(ObjectID id, ObjectDistances& objects) {
    ...
    void
    insertBKNNGNode(ObjectID id, ObjectDistances& results) {
    ...
    void
    insertKNNGNode(ObjectID id, ObjectDistances& results) {
        repository.insert(id, results);
    }

    void
    insertANNGNode(ObjectID id, ObjectDistances& results) {
        repository.insert(id, results);
    ...
    void
    insertIANNGNode(ObjectID id, ObjectDistances& results) {
        repository.insert(id, results);
        for (ObjectDistances::iterator ri = results.begin(); ri != results.end(); ri++) {
            assert(id != (*ri).id);
    ...
    void
    insertONNGNode(ObjectID id, ObjectDistances& results) {
    ...
    void
    removeEdgesReliably(ObjectID id);

    int
    truncateEdgesOptimally(ObjectID id, GraphNode& results, size_t truncationSize);

    int
    truncateEdges(ObjectID id) {
    ...
    // setup edgeSize
    inline size_t
    getEdgeSize(NGT::SearchContainer& sc) {
    ...
    void
    search(NGT::SearchContainer& sc, ObjectDistances& seeds);
    // for milvus
    void
    search(NGT::SearchContainer& sc, ObjectDistances& seeds, const faiss::BitsetView bitset);

#ifdef NGT_GRAPH_READ_ONLY_GRAPH
    template <typename COMPARATOR, typename CHECK_LIST>
    void
    searchReadOnlyGraph(NGT::SearchContainer& sc, ObjectDistances& seeds);
#endif

    void
    removeEdge(ObjectID fid, ObjectID rmid) {
    ...
    void
    removeEdge(GraphNode& node, ObjectDistance& edge) {
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
    ...
    class BooleanVector : public std::vector<bool> {
     public:
        ...
        inline void
        insert(size_t i) {
            std::vector<bool>::operator[](i) = true;
        }
    };
...
class DistanceCheckedSet : public unordered_set<ObjectID> {
 public:
    ...
    class NodeWithPosition : public ObjectDistance {
     public:
        ...
        NodeWithPosition&
        operator=(const NodeWithPosition& n) {
...
#endif
    void
    setupDistances(NGT::SearchContainer& sc, ObjectDistances& seeds);
    void
    setupDistances(NGT::SearchContainer& sc, ObjectDistances& seeds,
                   double (&comparator)(const void*, const void*, size_t));

    void
    setupSeeds(SearchContainer& sc, ObjectDistances& seeds, ResultSet& results, UncheckedSet& unchecked,
    ...
    int
    getEdgeSize() {
        return property.edgeSizeForCreation;
    }

    ObjectRepository&
    getObjectRepository() {
        return objectSpace->getRepository();
    }
 ...
 public:
    void
    addEdge(GraphNode& node, ObjectID addID, Distance addDistance, bool identityCheck = true) {
    ...
    bool
    addEdge(ObjectID target, ObjectID addID, Distance addDistance, bool identityCheck = true) {
        size_t minsize = 0;
        ...
        if ((size_t)property.truncationThreshold != 0 && node.size() - minsize > (size_t)property.truncationThreshold) {
            return true;
        }
        return false;
    }
    ...
    addEdgeDeletingExcessEdges(ObjectID target, ObjectID addID, Distance addDistance, bool identityCheck = true) {
        ...
        if (node.size() > kEdge && node.at(kEdge, repository.allocator).distance >= addDistance) {
            ...
            if ((linkedNode.size() > kEdge) &&
                node.at(kEdge, repository.allocator).distance >= linkedNode.at(kEdge, repository.allocator).distance) {
#else
        if (node.size() > kEdge && node[kEdge].distance >= addDistance) {
            ...
            if ((linkedNode.size() > kEdge) && node[kEdge].distance >= linkedNode[kEdge].distance) {
                ...
                try {
                    ...
                    std::stringstream msg;
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                    msg << "addEdge: Cannot remove. (a) " << target << "," << addID << ","
                        << node.at(kEdge, repository.allocator).id << ","
                        << node.at(kEdge, repository.allocator).distance;
#else
...
#ifdef NGT_GRAPH_READ_ONLY_GRAPH
    void
    loadSearchGraph(const std::string& database) {
 ...
 public:
    virtual int64_t
    memSize() {
        return repository.memSize() + searchRepository.memSize() + property.memSize() + objectSpace->memSize();
    }

...
class DVPTree {
 public:
    ...
    void
    open(const std::string& f, size_t sharedMemorySize) {
    ...
    void
    insert(InsertContainer& iobj);

    void
    insert(InsertContainer& iobj, LeafNode* n);

    Node::ID
    split(InsertContainer& iobj, LeafNode& leaf);

    Node::ID
    recombineNodes(InsertContainer& ic, Node::Objects& fs, LeafNode& leaf);

    void
    insertObject(InsertContainer& obj, LeafNode& leaf);

    typedef std::stack<Node::ID> UncheckedNode;

    void
    search(SearchContainer& so);
    void
    search(SearchContainer& so, InternalNode& node, UncheckedNode& uncheckedNode);
    void
    search(SearchContainer& so, LeafNode& node, UncheckedNode& uncheckedNode);

    bool
    searchObject(ObjectID id) {
    ...
    void
    replace(ObjectID id, ObjectID replacedId) {
        remove(id, replacedId);
    }

    // remove the specified object.
    void
    remove(ObjectID id, ObjectID replaceId = 0) {
        LeafNode& ln = getLeaf(id);
        try {
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
    ...
    void
    removeNaively(ObjectID id, ObjectID replaceId = 0) {
        for (size_t i = 0; i < leafNodes.size(); i++) {
            if (leafNodes[i] != 0) {
                try {
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
    ...
    void
    removeEmptyNodes(InternalNode& node);

    Node::Objects*
    getObjects(LeafNode& n, Container& iobj);
    ...
    void
    getObjectIDsFromLeaf(Node::ID nid, ObjectDistances& rl) {
        LeafNode& ln = *(LeafNode*)getNode(nid);
        rl.clear();
        ObjectDistance r;
        for (size_t i = 0; i < ln.getObjectSize(); i++) {
    ...
    void
    getAllLeafNodeIDs(std::vector<Node::ID>& leafIDs) {
        leafIDs.clear();
    ...
    bool
    verify(size_t objCount, std::vector<uint8_t>& status) {
    ...
    size_t
    getSharedMemorySize(std::ostream& os, SharedMemoryAllocator::GetMemorySizeType t) {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
    ...
    void
    getAllObjectIDs(std::set<ObjectID>& ids) {
        for (size_t i = 0; i < leafNodes.size(); i++) {
            if (leafNodes[i] != 0) {
    ...
    virtual int64_t
    memSize() {
        return sizeof(size_t) * 2 + sizeof(splitMode) + name.size() + leafNodes.memSize() + internalNodes.memSize();
    }

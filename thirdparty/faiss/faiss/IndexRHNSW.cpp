...
void IndexRHNSW::get_sorted_access_counts(
...
void IndexRHNSW::set_target_level(const int tl) {
    hnsw.target_level = tl;
}

void IndexRHNSW::update_stats(
        idx_t n,
...
#pragma omp for
    ...
    //        for (auto i = 0; i < n; ++ i) {
    //            for (auto j = 0; j < query_stats[i].access_points.size(); ++
    //            j) {
    //                auto tgt =
    //                stats.access_cnt.find(query_stats[i].access_points[j]); if
    //                (tgt == stats.access_cnt.end())
    //                    stats.access_cnt[query_stats[i].access_points[j]] = 1;
    //                else
    //                    tgt->second += 1;
    //            }
...
size_t IndexRHNSWFlat::cal_size() {
    return IndexRHNSW::cal_size() +
            dynamic_cast<IndexFlat*>(storage)->cal_size();
}
...
IndexRHNSWSQ::IndexRHNSWSQ(int d, QuantizerType qtype, int M, MetricType metric)
...
size_t IndexRHNSWSQ::cal_size() {
    return IndexRHNSW::cal_size() +
            dynamic_cast<IndexScalarQuantizer*>(storage)->cal_size();
}
...
size_t IndexRHNSW2Level::cal_size() {
    return IndexRHNSW::cal_size() +
            dynamic_cast<Index2Layer*>(storage)->cal_size();
}

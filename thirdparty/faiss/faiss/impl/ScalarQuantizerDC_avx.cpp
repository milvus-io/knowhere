...
SQDistanceComputer* sq_get_distance_computer_avx(
    ...
    if (metric == METRIC_L2) {
        if (dim % 8 == 0) {
            return select_distance_computer_avx<SimilarityL2_avx<8>>(
                    qtype, dim, trained);
        } else {
            return select_distance_computer_avx<SimilarityL2_avx<1>>(
                    qtype, dim, trained);
        }
    } else {
        if (dim % 8 == 0) {
            return select_distance_computer_avx<SimilarityIP_avx<8>>(
                    qtype, dim, trained);
        } else {
            return select_distance_computer_avx<SimilarityIP_avx<1>>(
                    qtype, dim, trained);
        }
    }
}

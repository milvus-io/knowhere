...
__global__ void
ivfpqAppend(Tensor<int, 1, true> listIds,
            Tensor<int, 1, true> listOffset,
...
void
runIVFPQAppend(Tensor<int, 1, true>& listIds,
               Tensor<int, 1, true>& listOffset,
               Tensor<uint8_t, 2, true>& encodings,
               thrust::device_vector<void*>& listCodes,
               cudaStream_t stream) {
...
template <typename Codec>
__global__ void
sqEncode(Tensor<float, 2, true> vecs,
...
template <typename Codec>
void
runSQEncode(Tensor<float, 2, true>& vecs,
            Tensor<typename Codec::EncodeT, 2, true>& encodedVecs,
            Codec codec,
            cudaStream_t stream) {
...
ivfInterleavedAppend(
  ...
  for (; warpVec < listVecStart + numVecsAdding;
    ...
    for (int dim = 0; dim < encodedVecs.getSize(1); ++dim) {
      ...
      WarpPackedBits<EncodeT, EncodeBits>::write(laneId, enc, valid, data);
...
void
runIVFPQInterleavedAppend(Tensor<int, 1, true>& listIds,
                          Tensor<int, 1, true>& listOffset,
                          Tensor<int, 1, true>& uniqueLists,
                          Tensor<int, 1, true>& vectorsByUniqueList,
                          Tensor<int, 1, true>& uniqueListVectorStart,

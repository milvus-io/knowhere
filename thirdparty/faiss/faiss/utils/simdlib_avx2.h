...
static int popcnt_AVX2_lookup(const uint8_t* data, const size_t n);
...
#define ITER                                                           \
    {                                                                  \
        const __m256i vec = _mm256_loadu_si256(                        \
    ...
    while (i + 8 * 32 <= n) {
        ...
        ITER ITER ITER ITER ITER ITER ITER ITER acc = _mm256_add_epi64(
...
static int xor_popcnt_AVX2_lookup(
        const uint8_t* data1,
        const uint8_t* data2,
        const size_t n) {
    size_t i = 0;
...
static int or_popcnt_AVX2_lookup(
        const uint8_t* data1,
        const uint8_t* data2,
        const size_t n) {
    size_t i = 0;
...
static int and_popcnt_AVX2_lookup(
        const uint8_t* data1,
        const uint8_t* data2,
        const size_t n) {
    size_t i = 0;

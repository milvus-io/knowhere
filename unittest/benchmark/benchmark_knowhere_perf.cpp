...
class Benchmark_knowhere_perf : public ::testing::Test {
 public:
    ...
    int32_t CalcRecall(const idx_t* ids, int32_t nq_start, int32_t step, int32_t k) {
        assert(nq_start + step <= 10000);
        int32_t min_k = std::min(gt_k_, k);
        int32_t hit = 0;
        for (int32_t i = 0; i < step; i++) {
            std::unordered_set<idx_t> ground(gt_ids_ + (i + nq_start) * gt_k_, gt_ids_ + (i + nq_start) * gt_k_ + min_k);
    ...
    void test_idmap(const knowhere::Config& cfg) {
        ...
        for (int32_t i = 0; i + NQ_STEP_ <= GT_NQ_; i = (i + NQ_STEP_) % GT_NQ_) {
            knowhere::DatasetPtr ds_ptr = knowhere::GenDataset(NQ_STEP_, dim_, xq_ + (i * dim_));
            for (auto k : TOPKs_) {
                ...
                int32_t hit = CalcRecall(ids, i, NQ_STEP_, k);
                printf("  No.%4d: nq = [%4d, %4d), k = %4d, elapse = %.4fs, R@ = %.4f\n",
                       no++, i, i + NQ_STEP_, k, (t_end - t_start), (hit / float(NQ_STEP_ * std::min(gt_k_, k))));
            }
        }
        printf("================================================================================\n");
    ...
    void test_ivf(const knowhere::Config& cfg) {
        auto conf = cfg;
            ...
            for (auto nprobe : NPROBEs_) {
                ...
                for (auto k : TOPKs_) {
                    ...
                    printf("  No.%4d: nprobe = %4d, nq = [%4d, %4d), k = %4d, elapse = %.4fs, R@ = %.4f\n",
                           no++, nprobe, i, i + NQ_STEP_, k, (t_end - t_start), (hit / float(NQ_STEP_ * std::min(gt_k_, k))));
                }
            }
        }
        printf("================================================================================\n");
    ...
    void test_hnsw(const knowhere::Config& cfg) {
        auto conf = cfg;
            ...
            for (auto ef: EFs_) {
                ...
                for (auto k : TOPKs_) {
                    ...
                    printf("  No.%4d: ef = %4d, nq = [%4d, %4d), k = %4d, elapse = %.4fs, R@ = %.4f\n",
                           no++, ef, i, i + NQ_STEP_, k, (t_end - t_start), (hit / float(NQ_STEP_ * std::min(gt_k_, k))));
                }
            }
        }
        printf("================================================================================\n");
    ...
    void test_annoy(const knowhere::Config& cfg) {
        auto conf = cfg;
            ...
            for (auto sk: SEARCH_Ks_) {
                ...
                for (auto k : TOPKs_) {
                    ...
                    printf("  No.%4d: search_k = %4d, nq = [%4d, %4d), k = %4d, elapse = %.4fs, R@ = %.4f\n",
                           no++, sk, i, i + NQ_STEP_, k, (t_end - t_start), (hit / float(NQ_STEP_ * std::min(gt_k_, k))));
                }
            }
        }
        printf("================================================================================\n");

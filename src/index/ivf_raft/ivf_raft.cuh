...
postprocess_device_results(bool* enough_valid, int64_t* ids, float* dists, int64_t rows, int k, int target_k,
    ...
    for (auto row_index = blockIdx.x; row_index < rows; row_index += gridDim.x) {
        ...
        if (threadIdx.x == 0) {
            enough_valid[row_index] = (k - invalid_count >= target_k);
        }
    }
}
 ...
 public:
    ...
    Search(const DataSet& dataset, const Config& cfg, const BitsetView& bitset) const override {
        ...
        try {
            ...
            std::unique_ptr<struct raft_results> gpu_results;
            ...
            if constexpr (std::is_same_v<detail::raft_ivf_flat_index, T>) {
                ...
                gpu_results =
                    std::make_unique<struct raft_results>(RawSearch(*res_, raft::make_const_mdspan(data_gpu.view()), search_params,
                              std::min(ivf_raft_cfg.k + bitset.count(), (unsigned long)counts_), ivf_raft_cfg.k, gpu_bitset.view()));
            } else if constexpr (std::is_same_v<detail::raft_ivf_pq_index, T>) {
                ...
                gpu_results = std::make_unique<struct raft_results>(RawSearch(*res_, raft::make_const_mdspan(data_gpu.view()), search_params, ivf_raft_cfg.k,
            ...
            for (auto row_index = 0; row_index < gpu_results->ids.extent(0); ++row_index) {
                ...
                auto end =
                    thrust::device_pointer_cast(gpu_results->ids.data_handle() + gpu_results->ids.extent(1) * row_index + ivf_raft_cfg.k);
                thrust::copy(thrust::device.on(res_->get_stream().value()), begin, end, ids.get() + row_index * ivf_raft_cfg.k);
                auto dists_begin =
                ...
                auto dists_end = thrust::device_pointer_cast(gpu_results->dists.data_handle() +
                                                            gpu_results->dists.extent(1) * row_index + ivf_raft_cfg.k);

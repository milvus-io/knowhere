...
class ConcurrentBitset {
 public:
  ...
  explicit ConcurrentBitset(size_t count, const uint8_t* data)
      : count_(count), bitset_(((count + 8 - 1) >> 3)) {
  ...
  std::shared_ptr<ConcurrentBitset> operator&(const ConcurrentBitset& bitset) const {
  ...
  size_t size() const { return ((count_ + 8 - 1) >> 3); }

  const uint8_t* data() const { return reinterpret_cast<const uint8_t*>(bitset_.data()); }

  uint8_t* mutable_data() { return reinterpret_cast<uint8_t*>(bitset_.data()); }
...
class BitsetView {
 public:
  ...
  int64_t u8size() const { return (size_ + 8 - 1) >> 3; }

  const uint8_t* data() const { return blocks_; }

  uint8_t* mutable_data() { return const_cast<uint8_t*>(blocks_); }

...
void fvec_inner_products_ny(float* dis, const float* x, const float* y, size_t d, size_t ny) {
#define DISPATCH(dval)                              \
  case dval:                                        \
...
#endif
#if defined(__x86_64__)
float fvec_L1_sse(const float* x, const float* y, size_t d) { return fvec_L1_ref(x, y, d); }

float fvec_Linf_sse(const float* x, const float* y, size_t d) { return fvec_Linf_ref(x, y, d); }
...
float fvec_L2sqr(const float* x, const float* y, size_t d) { return fvec_L2sqr_ref(x, y, d); }

float fvec_L1(const float* x, const float* y, size_t d) { return fvec_L1_ref(x, y, d); }

float fvec_Linf(const float* x, const float* y, size_t d) { return fvec_Linf_ref(x, y, d); }

float fvec_inner_product(const float* x, const float* y, size_t d) {
  return fvec_inner_product_ref(x, y, d);
}
#if defined(__aarch64__)
float fvec_norm_L2sqr(const float* x, size_t d) { return fvec_norm_L2sqr_ref(x, d); }
...
static inline int fvec_madd_and_argmin_ref(size_t n, const float* a, float bf, const float* b,
                                           float* c) {
  float vmin = 1e20;
  int imin = -1;
...
static inline int fvec_madd_and_argmin_sse(size_t n, const float* a, float bf, const float* b,
                                           float* c) {
  n >>= 2;
  __m128 bf4 = _mm_set_ps1(bf);

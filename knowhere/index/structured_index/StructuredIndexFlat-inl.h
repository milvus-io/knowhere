...
template <typename T>
BinarySet StructuredIndexFlat<T>::Serialize(const milvus::knowhere::Config& config) {
  // TODO
  return BinarySet();
}

template <typename T>
void StructuredIndexFlat<T>::Load(const milvus::knowhere::BinarySet& index_binary) {}

template <typename T>
void StructuredIndexFlat<T>::Build(const size_t n, const T* values) {
  data_.reserve(n);
...
template <typename T>
const faiss::ConcurrentBitsetPtr StructuredIndexFlat<T>::In(const size_t n, const T* values) {
...
template <typename T>
const faiss::ConcurrentBitsetPtr StructuredIndexFlat<T>::NotIn(const size_t n, const T* values) {
...
template <typename T>
const faiss::ConcurrentBitsetPtr StructuredIndexFlat<T>::Range(const T value,
...
template <typename T>
const faiss::ConcurrentBitsetPtr StructuredIndexFlat<T>::Range(T lower_bound_value,
                                                               bool lb_inclusive,
                                                               T upper_bound_value,
                                                               bool ub_inclusive) {
  faiss::ConcurrentBitsetPtr bitset = std::make_shared<faiss::ConcurrentBitset>(data_.size());
  ...
  for (; lb < ub; ++lb) {
    if (lb_inclusive && ub_inclusive) {
      if (*lb >= IndexStructure<T>(lower_bound_value) &&
          *lb <= IndexStructure<T>(upper_bound_value)) {
        bitset->set(lb->idx_);
      }

...
BinarySet IndexAnnoy::Serialize(const Config& config) {
  if (!index_) {
...
void IndexAnnoy::Load(const BinarySet& index_binary) {
...
void IndexAnnoy::BuildAll(const DatasetPtr& dataset_ptr, const Config& config) {
  if (index_) {
    // it is builded all
...
DatasetPtr IndexAnnoy::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
#pragma omp parallel for
  for (unsigned int i = 0; i < rows; ++i) {
    ...
    distances.reserve(k);
    index_->get_nns_by_vector(static_cast<const float*>(p_data) + i * dim, k, search_k, &result,
                              &distances, bitset);
...
void IndexAnnoy::UpdateIndexSize() {
  if (!index_) {
    KNOWHERE_THROW_MSG("index not initialize");
  }

...
BinarySet BinaryIVF::Serialize(const Config& config) {
...
void BinaryIVF::Load(const BinarySet& index_binary) {
...
DatasetPtr BinaryIVF::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
void BinaryIVF::UpdateIndexSize() {
  if (!index_) {
    KNOWHERE_THROW_MSG("index not initialize");
  }
...
void BinaryIVF::Train(const DatasetPtr& dataset_ptr, const Config& config) {
...
void BinaryIVF::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_ || !index_->is_trained) {
    KNOWHERE_THROW_MSG("index not initialize");
  }
...
std::shared_ptr<faiss::IVFSearchParameters> BinaryIVF::GenParams(const Config& config) {
...
void BinaryIVF::QueryImpl(int64_t n, const uint8_t* data, int64_t k, float* distances,

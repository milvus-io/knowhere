...
#define CheckIntByRange(key, min, max)                                        \
  if (!oricfg.contains(key) || !oricfg[key].is_number_integer() ||            \
      oricfg[key].get<int64_t>() > max || oricfg[key].get<int64_t>() < min) { \
    return false;                                                             \
  }
...
#define CheckIntByValues(key, container)                                                   \
  if (!oricfg.contains(key) || !oricfg[key].is_number_integer()) {                         \
    ...
    auto finder =                                                                          \
        std::find(std::begin(container), std::end(container), oricfg[key].get<int64_t>()); \
...
#define CheckStrByValues(key, container)                                                       \
  if (!oricfg.contains(key) || !oricfg[key].is_string()) {                                     \
    ...
    auto finder =                                                                              \
        std::find(std::begin(container), std::end(container), oricfg[key].get<std::string>()); \
    if (finder == std::end(container)) {                                                       \
      return false;                                                                            \
    }                                                                                          \
  }

bool ConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::meta::DIM, DEFAULT_MIN_DIM, DEFAULT_MAX_DIM);
  CheckStrByValues(knowhere::Metric::TYPE, METRICS);
  return true;
}

bool ConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
...
int64_t MatchNlist(int64_t size, int64_t nlist) {
...
int64_t MatchNbits(int64_t size, int64_t nbits) {
  if (size < (1 << nbits)) {
...
bool IVFConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
...
bool IVFConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
...
bool IVFSQConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  oricfg[knowhere::IndexParams::nbits] = DEFAULT_NBITS;
  return IVFConfAdapter::CheckTrain(oricfg, mode);
}

bool IVFPQConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  ...
  auto rows = oricfg[knowhere::meta::ROWS].get<int64_t>();
  auto nbits =
      oricfg.count(IndexParams::nbits) ? oricfg[IndexParams::nbits].get<int64_t>() : DEFAULT_NBITS;
...
bool IVFPQConfAdapter::CheckPQParams(int64_t dimension, int64_t m, int64_t nbits, IndexMode& mode) {
#ifdef KNOWHERE_GPU_VERSION
  if (mode == knowhere::IndexMode::MODE_GPU &&
      !IVFPQConfAdapter::CheckGPUPQParams(dimension, m, nbits)) {
...
bool IVFPQConfAdapter::CheckGPUPQParams(int64_t dimension, int64_t m, int64_t nbits) {
  /*
   * Faiss 1.6
   * Only 1, 2, 3, 4, 6, 8, 10, 12, 16, 20, 24, 28, 32 dims per sub-quantizer are currently
   * supported with no precomputed codes. Precomputed codes supports any number of dimensions, but
   * will involve memory overheads.
   */
  static const std::vector<int64_t> support_dim_per_subquantizer{32, 28, 24, 20, 16, 12, 10,
                                                                 8,  6,  4,  3,  2,  1};
  static const std::vector<int64_t> support_subquantizer{96, 64, 56, 48, 40, 32, 28, 24,
                                                         20, 16, 12, 8,  4,  3,  2,  1};
...
bool IVFPQConfAdapter::CheckCPUPQParams(int64_t dimension, int64_t m) {
  return (dimension % m == 0);
}

bool IVFHNSWConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  // HNSW param check
  CheckIntByRange(knowhere::IndexParams::efConstruction, HNSW_MIN_EFCONSTRUCTION,
...
bool IVFHNSWConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  // HNSW param check
...
bool NSGConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  ...
  CheckIntByRange(knowhere::IndexParams::candidate, MIN_CANDIDATE_POOL_SIZE,
                  MAX_CANDIDATE_POOL_SIZE);

  // auto tune params
  oricfg[knowhere::IndexParams::nlist] =
      MatchNlist(oricfg[knowhere::meta::ROWS].get<int64_t>(), 8192);
...
bool NSGConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
...
bool HNSWConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::efConstruction, HNSW_MIN_EFCONSTRUCTION,
                  HNSW_MAX_EFCONSTRUCTION);
  CheckIntByRange(knowhere::IndexParams::M, HNSW_MIN_M, HNSW_MAX_M);
...
bool HNSWConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::ef, oricfg[knowhere::meta::TOPK], HNSW_MAX_EF);

  return ConfAdapter::CheckSearch(oricfg, type, mode);
}

bool RHNSWFlatConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::efConstruction, HNSW_MIN_EFCONSTRUCTION,
                  HNSW_MAX_EFCONSTRUCTION);
  CheckIntByRange(knowhere::IndexParams::M, HNSW_MIN_M, HNSW_MAX_M);
...
bool RHNSWFlatConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::ef, oricfg[knowhere::meta::TOPK], HNSW_MAX_EF);

  return ConfAdapter::CheckSearch(oricfg, type, mode);
}

bool RHNSWPQConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::efConstruction, HNSW_MIN_EFCONSTRUCTION,
                  HNSW_MAX_EFCONSTRUCTION);
  CheckIntByRange(knowhere::IndexParams::M, HNSW_MIN_M, HNSW_MAX_M);
  ...
  if (!IVFPQConfAdapter::CheckCPUPQParams(dimension,
                                          oricfg[knowhere::IndexParams::PQM].get<int64_t>())) {
    return false;
  }
...
bool RHNSWPQConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::ef, oricfg[knowhere::meta::TOPK], HNSW_MAX_EF);

  return ConfAdapter::CheckSearch(oricfg, type, mode);
}

bool RHNSWSQConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::efConstruction, HNSW_MIN_EFCONSTRUCTION,
                  HNSW_MAX_EFCONSTRUCTION);
  CheckIntByRange(knowhere::IndexParams::M, HNSW_MIN_M, HNSW_MAX_M);
...
bool RHNSWSQConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::ef, oricfg[knowhere::meta::TOPK], HNSW_MAX_EF);

  return ConfAdapter::CheckSearch(oricfg, type, mode);
}

bool BinIDMAPConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  static const std::vector<std::string> METRICS{
      knowhere::Metric::HAMMING, knowhere::Metric::JACCARD, knowhere::Metric::TANIMOTO,
      knowhere::Metric::SUBSTRUCTURE, knowhere::Metric::SUPERSTRUCTURE};
...
bool BinIVFConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  static const std::vector<std::string> METRICS{
      knowhere::Metric::HAMMING, knowhere::Metric::JACCARD, knowhere::Metric::TANIMOTO};
...
bool ANNOYConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
...
bool ANNOYConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::search_k, std::numeric_limits<int64_t>::min(),
                  std::numeric_limits<int64_t>::max());
  return ConfAdapter::CheckSearch(oricfg, type, mode);
}

bool NGTPANNGConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::edge_size, NGT_MIN_EDGE_SIZE, NGT_MAX_EDGE_SIZE);
  CheckIntByRange(knowhere::IndexParams::forcedly_pruned_edge_size, NGT_MIN_EDGE_SIZE,
                  NGT_MAX_EDGE_SIZE);
  CheckIntByRange(knowhere::IndexParams::selectively_pruned_edge_size, NGT_MIN_EDGE_SIZE,
                  NGT_MAX_EDGE_SIZE);
...
bool NGTPANNGConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
...
bool NGTONNGConfAdapter::CheckTrain(Config& oricfg, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::edge_size, NGT_MIN_EDGE_SIZE, NGT_MAX_EDGE_SIZE);
...
bool NGTONNGConfAdapter::CheckSearch(Config& oricfg, const IndexType type, const IndexMode mode) {
  CheckIntByRange(knowhere::IndexParams::max_search_edges, -1, NGT_MAX_EDGE_SIZE);
  CheckFloatByRange(knowhere::IndexParams::epsilon, -1.0, 1.0);
  return ConfAdapter::CheckSearch(oricfg, type, mode);
}

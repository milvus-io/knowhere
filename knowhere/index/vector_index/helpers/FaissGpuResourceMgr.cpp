...
void FaissGpuResourceMgr::AllocateTempMem(ResPtr& resource, const int64_t device_id,
                                          const int64_t size) {
  if (size) {
...
void FaissGpuResourceMgr::InitDevice(int64_t device_id, int64_t pin_mem_size, int64_t temp_mem_size,
                                     int64_t res_num) {
  DeviceParams params;
  ...
  LOG_KNOWHERE_DEBUG_ << "DEVICEID " << device_id << ", pin_mem_size " << pin_mem_size / MB
                      << "MB, temp_mem_size " << temp_mem_size / MB << "MB, resource count "
                      << res_num;
}
...
ResPtr FaissGpuResourceMgr::GetRes(const int64_t device_id, const int64_t alloc_size) {
  InitResource();

...
void DynamicResultSet::AlloctionImpl() {
  if (count <= 0) {
...
void DynamicResultSet::SortImpl(ResultSetPostProcessType postProcessType) {
...
template <bool asc>
void DynamicResultSet::quick_sort(size_t lp, size_t rp) {
  auto len = rp - lp;
  if (len <= 1) {
    return;
  }
...
DynamicResultSet DynamicResultCollector::Merge(size_t limit,
                                               ResultSetPostProcessType postProcessType) {
  if (limit <= 0) {
...
#pragma omp parallel for
  for (auto i = 0; i < pos; ++i) {
    for (auto& pseg : seg_results[i]) {
      ...
      pseg->copy_range(0, len, ret.labels.get() + boundaries[i],
                       ret.distances.get() + boundaries[i]);
      boundaries[i] += len;
    }
  }
  // partial copy
  ...
  for (auto& pseg : seg_results[pos]) {
    ...
    pseg->copy_range(0, ncopy, ret.labels.get() + boundaries[pos],
                     ret.distances.get() + boundaries[pos]);

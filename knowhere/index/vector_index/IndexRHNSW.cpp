...
BinarySet IndexRHNSW::Serialize(const Config& config) {
  if (!index_) {
...
void IndexRHNSW::Load(const BinarySet& index_binary) {
  try {
...
void IndexRHNSW::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  KNOWHERE_THROW_MSG(
      "IndexRHNSW has no implementation of Train, please use IndexRHNSW(Flat/SQ/PQ) instead!");
}

void IndexRHNSW::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_) {
...
DatasetPtr IndexRHNSW::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
void IndexRHNSW::UpdateIndexSize() {
  KNOWHERE_THROW_MSG(
      "IndexRHNSW has no implementation of UpdateIndexSize, please use IndexRHNSW(Flat/SQ/PQ) "
      "instead!");
}

...
std::shared_ptr<SPTAG::MetadataSet> ConvertToMetadataSet(const DatasetPtr& dataset_ptr) {
  ...
  std::shared_ptr<SPTAG::MetadataSet> metaset(new SPTAG::MemMetadataSet(
      SPTAG::ByteArray((std::uint8_t*)p_id, elems * sizeof(int64_t), true),
      SPTAG::ByteArray((std::uint8_t*)p_offset, elems * sizeof(int64_t), true), elems));

  return metaset;
}

std::shared_ptr<SPTAG::VectorSet> ConvertToVectorSet(const DatasetPtr& dataset_ptr) {
...
std::vector<SPTAG::QueryResult> ConvertToQueryResult(const DatasetPtr& dataset_ptr,
                                                     const Config& config) {
  GET_TENSOR_DATA_DIM(dataset_ptr);
...
DatasetPtr ConvertToDataset(std::vector<SPTAG::QueryResult> query_results,

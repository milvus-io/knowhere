...
void NsgIndex::Build(size_t nb, float* data, const int64_t* ids, const BuildParams& parameters) {
  ntotal = nb;
  ...
  LOG_KNOWHERE_DEBUG_ << "Graph physical size: " << total_degree * sizeof(node_t) / 1024 / 1024
                      << "m";
...
void NsgIndex::InitNavigationPoint(float* data) {
...
// Specify Link
void NsgIndex::GetNeighbors(const float* query, float* data, std::vector<Neighbor>& resset,
    ...
    while (cursor < buffer_size) {
      ...
      if (!resset[cursor].has_explored) {
        ...
        for (node_t id : wait_for_search_node_vec) {
          ...
          size_t pos =
              InsertIntoPool(resset.data(), buffer_size, nn);  // replace with a closer node
...
void NsgIndex::GetNeighbors(const float* query, float* data, std::vector<Neighbor>& resset,
                            Graph& graph, SearchParams* params) {
...
void NsgIndex::SyncPrune(float* data, size_t n, std::vector<Neighbor>& pool,
                         boost::dynamic_bitset<>& has_calculated, float* cut_graph_dist) {
...
void NsgIndex::InterInsert(float* data, unsigned n, std::vector<std::mutex>& mutex_vec,
  ...
  for (size_t i = 0; i < out_degree; ++i) {
    ...
    std::vector<Neighbor>
        wait_for_link_pool;  // maintain candidate neighbor of the current neighbor.
    int duplicate = false;
    {
...
void NsgIndex::SelectEdge(float* data, unsigned& cursor, std::vector<Neighbor>& sort_pool,
                          std::vector<Neighbor>& result, bool limit) {
  auto& pool = sort_pool;
...
void NsgIndex::CheckConnectivity(float* data) {
...
void NsgIndex::FindUnconnectedNode(float* data, boost::dynamic_bitset<>& has_linked,
                                   int64_t& root) {
...
//                     float dist = distance_->Compare(query, ori_data_ + dimension * id,
//                     dimension);
...
//                     size_t pos = InsertIntoPool(resset.data(), buffer_size, nn);  // replace with
//                     a closer node if (pos < nearest_updated_pos)
//                         nearest_updated_pos = pos;

//                     //>> Debug code
//                     /////
//                     // std::cout << "pos: " << pos << ", nn: " << nn.id << ":" << nn.distance <<
//                     ", nup: " <<
//                     // nearest_updated_pos << std::endl;
//                     /////
...
// void
// NsgIndex::Search(const float* query, const unsigned& nq, const unsigned& dim, const unsigned& k,
// float* dist,
...
void NsgIndex::Search(const float* query, float* data, const unsigned& nq, const unsigned& dim,
                      const unsigned& k, float* dist, int64_t* ids, SearchParams& params,
...
void NsgIndex::SetKnnGraph(Graph& g) { knng = std::move(g); }

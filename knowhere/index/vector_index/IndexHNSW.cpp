...
BinarySet IndexHNSW::Serialize(const Config& config) {
  if (!index_) {
...
void IndexHNSW::Load(const BinarySet& index_binary) {
  try {
...
void IndexHNSW::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  try {
    ...
    index_ = std::make_shared<hnswlib::HierarchicalNSW<float>>(
        space, rows, config[IndexParams::M].get<int64_t>(),
...
void IndexHNSW::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_) {
...
DatasetPtr IndexHNSW::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
void IndexHNSW::UpdateIndexSize() {
  if (!index_) {
    KNOWHERE_THROW_MSG("index not initialize");
  }

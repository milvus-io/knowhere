...
BinarySet BinaryIDMAP::Serialize(const Config& config) {
  if (!index_) {
...
void BinaryIDMAP::Load(const BinarySet& index_binary) {
  Assemble(const_cast<BinarySet&>(index_binary));
  LoadImpl(index_binary, index_type_);
}

DatasetPtr BinaryIDMAP::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
DynamicResultSegment BinaryIDMAP::QueryByDistance(const milvus::knowhere::DatasetPtr& dataset,
  ...
  real_idx->range_search(rows, reinterpret_cast<const uint8_t*>(p_data), radius, res, buffer_size,
                         bitset);
...
void BinaryIDMAP::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_) {
    KNOWHERE_THROW_MSG("index not initialize");
  }
...
void BinaryIDMAP::Train(const DatasetPtr& dataset_ptr, const Config& config) {
...
const uint8_t* BinaryIDMAP::GetRawVectors() {
  try {
...
void BinaryIDMAP::QueryImpl(int64_t n, const uint8_t* data, int64_t k, float* distances,

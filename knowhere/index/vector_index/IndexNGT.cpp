...
BinarySet IndexNGT::Serialize(const Config& config) {
  if (!index_) {
...
void IndexNGT::Load(const BinarySet& index_binary) {
...
void IndexNGT::BuildAll(const DatasetPtr& dataset_ptr, const Config& config) {
  KNOWHERE_THROW_MSG(
      "IndexNGT has no implementation of BuildAll, please use IndexNGT(PANNG/ONNG) instead!");
}
...
DatasetPtr IndexNGT::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
#pragma omp parallel for
  for (unsigned int i = 0; i < rows; ++i) {
    ...
    if (index_->getObjectSpace().getDistanceType() ==
        NGT::ObjectSpace::DistanceType::DistanceTypeIP) {
      dis_coefficient = -1.0;
    }
...
void IndexNGT::UpdateIndexSize() {
  KNOWHERE_THROW_MSG(
      "IndexNGT has no implementation of UpdateIndexSize, please use IndexNGT(PANNG/ONNG) "
      "instead!");
}

...
IndexRHNSWSQ::IndexRHNSWSQ(int d, faiss::QuantizerType qtype, int M,
                           milvus::knowhere::MetricType metric) {
  faiss::MetricType mt =
...
BinarySet IndexRHNSWSQ::Serialize(const Config& config) {
  if (!index_) {
...
void IndexRHNSWSQ::Load(const BinarySet& index_binary) {
  try {
...
void IndexRHNSWSQ::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  try {
    GET_TENSOR_DATA_DIM(dataset_ptr)
    faiss::MetricType metric_type = GetMetricType(config[Metric::TYPE].get<std::string>());
    int32_t efConstruction = config[IndexParams::efConstruction];

    auto idx = new faiss::IndexRHNSWSQ(int(dim), faiss::QuantizerType::QT_8bit,
                                       config[IndexParams::M], metric_type);
...
void IndexRHNSWSQ::UpdateIndexSize() {
  if (!index_) {

...
inline int len_of_pow2(uint64_t x) { return __builtin_popcountl(x - 1); }
...
class Statistics {
 public:
   ...
   */
  const std::string& IndexType() { return index_type; }
   ...
   */
  const std::vector<size_t>& FilterHistograms() { return filter_stat; }

  std::unique_lock<std::mutex> Lock() { return std::unique_lock<std::mutex>(update_lock); }

 public:
  void update_nq(const int64_t nq) {
    // batch
    batch_cnt++;
  ...
  void update_total_query_time(const double query_time) { total_query_time += query_time; }

  void update_filter_percentage(const faiss::BitsetView bitset) {
...
class HNSWStatistics : public Statistics {
 public:
   ...
   * @retval: none
   */
  const std::vector<size_t>& LevelNodesNum() { return distribution; }
   ...
   * @retval: Access CDF
   */
  virtual std::vector<double> AccessCDF() {
   ...
   * @retval: Access CDF
   */
  virtual std::vector<double> AccessCDF(const std::vector<size_t>& axis_x) = 0;

 public:
  void update_ef_sum(const int64_t ef) { ef_sum += ef; }

  void update_level_distribution(const int max_level, const std::vector<int>& levels) {
...
class LibHNSWStatistics : public HNSWStatistics {
 public:
  ...
  std::vector<double> AccessCDF(const std::vector<size_t>& axis_x) override;
...
class IVFStatistics : public Statistics {
 public:
   ...
   * @retval: nprobe
   */
  int64_t Nlist() { return nlist; }

  /*
   * Get the statistics of the search parameter nprboe (count of batches)  (Level 1)
   * @retval: <nprobe, count>
   */
  std::unordered_map<int64_t, size_t> SearchNprobe() {
    auto lock = Lock();
   ...
   * @retval: Access CDF
   */
  std::vector<double> AccessCDF(const std::vector<size_t>& axis_x);

 public:
  void count_nprobe(const int64_t nprobe);

  void update_ivf_access_stats(const std::vector<size_t>& nprobe_statistics);

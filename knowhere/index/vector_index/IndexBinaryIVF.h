...
class BinaryIVF : public VecIndex, public FaissBaseBinaryIndex {
 public:
  ...
  explicit BinaryIVF(std::shared_ptr<faiss::IndexBinary> index)
  ...
  void Train(const DatasetPtr& dataset_ptr, const Config& config) override;

  void AddWithoutIds(const DatasetPtr&, const Config&) override;

  DatasetPtr Query(const DatasetPtr& dataset_ptr, const Config& config,

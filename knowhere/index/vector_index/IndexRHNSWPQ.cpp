...
BinarySet IndexRHNSWPQ::Serialize(const Config& config) {
  if (!index_) {
...
void IndexRHNSWPQ::Load(const BinarySet& index_binary) {
  try {
...
void IndexRHNSWPQ::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  try {
...
void IndexRHNSWPQ::UpdateIndexSize() {
  if (!index_) {

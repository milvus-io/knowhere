...
void IVFSQHybrid::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  if (gpu_res != nullptr) {
    ...
    index_ = std::make_shared<faiss::gpu::GpuIndexIVFSQHybrid>(gpu_res->faiss_res.get(), dim, nlist,
                                                               faiss::QuantizerType::QT_8bit,
                                                               metric_type, true, idx_config);
...
VecIndexPtr IVFSQHybrid::CopyGpuToCpu(const Config& config) {
  if (gpu_mode_ == 0) {
...
VecIndexPtr IVFSQHybrid::CopyCpuToGpu(const int64_t device_id, const Config& config) {
  if (auto res = FaissGpuResourceMgr::GetInstance().GetRes(device_id)) {
    ...
    auto gpu_index =
        faiss::gpu::index_cpu_to_gpu(res->faiss_res.get(), device_id, index_.get(), &option);
    ...
    KNOWHERE_THROW_MSG("CopyCpuToGpu Error, can't get gpu: " + std::to_string(gpu_id_) +
                       "resource");
  }
}

std::pair<VecIndexPtr, FaissIVFQuantizerPtr> IVFSQHybrid::CopyCpuToGpuWithQuantizer(
    const int64_t device_id, const Config& config) {
  if (auto res = FaissGpuResourceMgr::GetInstance().GetRes(device_id)) {
    ResScope rs(res, device_id, false);
    faiss::gpu::GpuClonerOptions option;
    option.allInGpu = true;
    ...
    auto gpu_index =
        faiss::gpu::index_cpu_to_gpu(res->faiss_res.get(), device_id, &index_composition, &option);
    ...
    q->size =
        index_composition.quantizer->d * index_composition.quantizer->getNumVecs() * sizeof(float);
    q->gpu_id = device_id;
...
VecIndexPtr IVFSQHybrid::LoadData(const FaissIVFQuantizerPtr& quantizer_ptr, const Config& config) {
  ...
  if (auto res = FaissGpuResourceMgr::GetInstance().GetRes(gpu_id)) {
    ...
    auto gpu_index =
        faiss::gpu::index_cpu_to_gpu(res->faiss_res.get(), gpu_id, &index_composition, &option);
...
FaissIVFQuantizerPtr IVFSQHybrid::LoadQuantizer(const Config& config) {
...
void IVFSQHybrid::SetQuantizer(const FaissIVFQuantizerPtr& quantizer_ptr) {
...
void IVFSQHybrid::UnsetQuantizer() {
...
BinarySet IVFSQHybrid::SerializeImpl(const IndexType& type) {
...
void IVFSQHybrid::LoadImpl(const BinarySet& binary_set, const IndexType& type) {
...
void IVFSQHybrid::QueryImpl(int64_t n, const float* data, int64_t k, float* distances,
                            int64_t* labels, const Config& config, const faiss::BitsetView bitset) {
  if (gpu_mode_ == 2) {
    ...
    if (auto res = FaissGpuResourceMgr::GetInstance().GetRes(gpu_id)) {
      ...
      KNOWHERE_THROW_MSG("Hybrid Search Error, can't get gpu: " + std::to_string(gpu_id) +
                         "resource");
    }
  } else if (gpu_mode_ == 0) {
    IVF::QueryImpl(n, data, k, distances, labels, config, bitset);
...
void IVFSQHybrid::UpdateIndexSize() {
  if (!index_) {
  ...
  index_size_ =
      nb * code_size + nb * sizeof(int64_t) + 2 * d * sizeof(float) + nlist * d * sizeof(float);
}

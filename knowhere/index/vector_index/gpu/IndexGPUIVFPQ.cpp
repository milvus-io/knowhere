...
void GPUIVFPQ::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  if (gpu_res != nullptr) {
    ...
    index_ = std::make_shared<faiss::gpu::GpuIndexIVFPQ>(gpu_res->faiss_res.get(), dim, nlist, m,
                                                         nbits, metric_type, idx_config);
...
VecIndexPtr GPUIVFPQ::CopyGpuToCpu(const Config& config) {

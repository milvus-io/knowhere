...
VecIndexPtr GPUIDMAP::CopyGpuToCpu(const Config& config) {
...
BinarySet GPUIDMAP::SerializeImpl(const IndexType& type) {
  try {
...
void GPUIDMAP::LoadImpl(const BinarySet& index_binary, const IndexType& type) {
...
VecIndexPtr GPUIDMAP::CopyGpuToGpu(const int64_t device_id, const Config& config) {
  auto cpu_index = CopyGpuToCpu(config);
  return std::static_pointer_cast<IDMAP>(cpu_index)->CopyCpuToGpu(device_id, config);
}

const float* GPUIDMAP::GetRawVectors() { KNOWHERE_THROW_MSG("Not support"); }

void GPUIDMAP::QueryImpl(int64_t n, const float* data, int64_t k, float* distances, int64_t* labels,
...
void GPUIDMAP::GenGraph(const float* data, const int64_t k, GraphType& graph,
                        const Config& config) {
  int64_t K = k + 1;
  auto ntotal = Count();

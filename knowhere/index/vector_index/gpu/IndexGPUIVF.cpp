...
void GPUIVF::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  if (gpu_res != nullptr) {
    ...
    index_ = std::make_shared<faiss::gpu::GpuIndexIVFFlat>(gpu_res->faiss_res.get(), dim, nlist,
...
void GPUIVF::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
...
VecIndexPtr GPUIVF::CopyGpuToCpu(const Config& config) {
...
VecIndexPtr GPUIVF::CopyGpuToGpu(const int64_t device_id, const Config& config) {
  auto host_index = CopyGpuToCpu(config);
  return std::static_pointer_cast<IVF>(host_index)->CopyCpuToGpu(device_id, config);
}

BinarySet GPUIVF::SerializeImpl(const IndexType& type) {
...
void GPUIVF::LoadImpl(const BinarySet& binary_set, const IndexType& type) {
...
void GPUIVF::QueryImpl(int64_t n, const float* data, int64_t k, float* distances, int64_t* labels,
                       const Config& config, const faiss::BitsetView bitset) {
  auto device_index = std::dynamic_pointer_cast<faiss::gpu::GpuIndexIVF>(index_);
  if (device_index) {
    device_index->nprobe =
        std::min(static_cast<int>(config[IndexParams::nprobe]), device_index->nlist);
    ResScope rs(res_, gpu_id_);
    ...
    for (int64_t i = 0; i < n; i += block_size) {
      ...
      device_index->search(search_size, reinterpret_cast<const float*>(data) + i * dim, k,
                           distances + i * k, labels + i * k, bitset);
    }
  } else {

...
void GPUIVFSQ::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  if (gpu_res != nullptr) {
    ...
    index_ = std::make_shared<faiss::gpu::GpuIndexIVFScalarQuantizer>(
        gpu_res->faiss_res.get(), dim, nlist, faiss::QuantizerType::QT_8bit, metric_type, true,
        idx_config);
...
VecIndexPtr GPUIVFSQ::CopyGpuToCpu(const Config& config) {

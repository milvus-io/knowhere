...
CPUSPTAGRNG::Serialize(const Config& config) {
    ...
    std::shared_ptr<uint8_t> sample(nullptr, std::default_delete<uint8_t[]>());
    sample.reset(static_cast<uint8_t*>(index_blobs[0].Data()));
    std::shared_ptr<uint8_t> tree(nullptr, std::default_delete<uint8_t[]>());
    tree.reset(static_cast<uint8_t*>(index_blobs[1].Data()));
    std::shared_ptr<uint8_t> graph(nullptr, std::default_delete<uint8_t[]>());
    graph.reset(static_cast<uint8_t*>(index_blobs[2].Data()));
    std::shared_ptr<uint8_t> deleteid(nullptr, std::default_delete<uint8_t[]>());
    deleteid.reset(static_cast<uint8_t*>(index_blobs[3].Data()));
    std::shared_ptr<uint8_t> metadata1(nullptr, std::default_delete<uint8_t[]>());
    metadata1.reset(static_cast<uint8_t*>(index_blobs[4].Data()));
    std::shared_ptr<uint8_t> metadata2(nullptr, std::default_delete<uint8_t[]>());
    metadata2.reset(static_cast<uint8_t*>(index_blobs[5].Data()));
    std::shared_ptr<uint8_t> x_cfg(nullptr, std::default_delete<uint8_t[]>());

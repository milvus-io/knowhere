...
CPUSPTAGRNG::CPUSPTAGRNG(const std::string& IndexType) {
  if (IndexType == "KDT") {
    ...
    index_ptr_ = SPTAG::VectorIndex::CreateInstance(SPTAG::IndexAlgoType::BKT,
                                                    SPTAG::VectorValueType::Float);
    index_ptr_->SetParameter("DistCalcMethod", "L2");
...
BinarySet CPUSPTAGRNG::Serialize(const Config& config) {
...
void CPUSPTAGRNG::Load(const BinarySet& binary_set) {
...
void CPUSPTAGRNG::BuildAll(const DatasetPtr& origin, const Config& train_config) {
...
void CPUSPTAGRNG::SetParameters(const Config& config) {
#define Assign(param_name, str_name) \
  ...
  if (index_type_ == IndexEnum::INDEX_SPTAG_KDT_RNT) {
    ...
    Assign("thresholdofnumberofcontinuousnobetterpropagation",
...
DatasetPtr CPUSPTAGRNG::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
void CPUSPTAGRNG::UpdateIndexSize() {
  if (!index_ptr_) {
    KNOWHERE_THROW_MSG("index not initialize");
  }

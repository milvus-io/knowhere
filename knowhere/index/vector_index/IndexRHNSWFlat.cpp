...
BinarySet IndexRHNSWFlat::Serialize(const Config& config) {
  if (!index_) {
  ...
  try {
    ...
    int64_t meta_info[3] = {real_idx->storage->metric_type, real_idx->storage->d,
                            real_idx->storage->ntotal};
...
void IndexRHNSWFlat::Load(const BinarySet& index_binary) {
  try {
    ...
    auto real_idx = dynamic_cast<faiss::IndexRHNSWFlat*>(index_.get());
    real_idx->storage = new faiss::IndexFlat(static_cast<faiss::idx_t>(meta_info[1]),
                                             static_cast<faiss::MetricType>(meta_info[0]));
...
void IndexRHNSWFlat::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  try {
...
void IndexRHNSWFlat::UpdateIndexSize() {
  if (!index_) {

...
BinarySet IVFHNSW::Serialize(const Config& config) {
...
void IVFHNSW::Load(const BinarySet& binary_set) {
  try {
...
void IVFHNSW::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  auto index = std::make_shared<faiss::IndexIVFFlat>(
      coarse_quantizer, dim, config[IndexParams::nlist].get<int64_t>(), metric_type);
  index->own_fields = true;
  index->train(rows, reinterpret_cast<const float*>(p_data));
  index_ = index;
}

VecIndexPtr IVFHNSW::CopyCpuToGpu(const int64_t device_id, const Config& config) {
  KNOWHERE_THROW_MSG("IVFHNSW::CopyCpuToGpu not supported.");
}

void IVFHNSW::UpdateIndexSize() {
  if (!index_) {
...
void IVFHNSW::QueryImpl(int64_t n, const float* data, int64_t k, float* distances, int64_t* labels,

...
BinarySet IDMAP::Serialize(const Config& config) {
  if (!index_) {
...
void IDMAP::Load(const BinarySet& binary_set) {
  Assemble(const_cast<BinarySet&>(binary_set));
  LoadImpl(binary_set, index_type_);
}

void IDMAP::Train(const DatasetPtr& dataset_ptr, const Config& config) {
...
void IDMAP::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_) {
    KNOWHERE_THROW_MSG("index not initialize");
  }
...
DatasetPtr IDMAP::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
DynamicResultSegment IDMAP::QueryByDistance(const milvus::knowhere::DatasetPtr& dataset,
  ...
  real_idx->range_search(rows, reinterpret_cast<const float*>(p_data), radius, res, buffer_size,
                         bitset);
...
VecIndexPtr IDMAP::CopyCpuToGpu(const int64_t device_id, const Config& config) {
#ifdef KNOWHERE_GPU_VERSION
...
const float* IDMAP::GetRawVectors() {
  try {
...
void IDMAP::QueryImpl(int64_t n, const float* data, int64_t k, float* distances, int64_t* labels,

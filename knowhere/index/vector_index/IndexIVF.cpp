...
BinarySet IVF::Serialize(const Config& config) {
...
void IVF::Load(const BinarySet& binary_set) {
...
void IVF::Train(const DatasetPtr& dataset_ptr, const Config& config) {
...
void IVF::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_ || !index_->is_trained) {
    KNOWHERE_THROW_MSG("index not initialize or trained");
  }
...
DatasetPtr IVF::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
VecIndexPtr IVF::CopyCpuToGpu(const int64_t device_id, const Config& config) {
#ifdef KNOWHERE_GPU_VERSION
...
void IVF::GenGraph(const float* data, const int64_t k, GraphType& graph, const Config& config) {
  int64_t K = k + 1;
  auto ntotal = Count();
...
void IVF::QueryImpl(int64_t n, const float* data, int64_t k, float* distances, int64_t* labels,

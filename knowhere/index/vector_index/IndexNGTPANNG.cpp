...
#include "index/vector_index/IndexNGTPANNG.h"
...
void IndexNGTPANNG::BuildAll(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  index_ = std::shared_ptr<NGT::Index>(
      NGT::Index::createGraphAndTree(reinterpret_cast<const float*>(p_data), prop, rows));

  auto forcedly_pruned_edge_size = config[IndexParams::forcedly_pruned_edge_size].get<int64_t>();
  auto selectively_pruned_edge_size =
      config[IndexParams::selectively_pruned_edge_size].get<int64_t>();

  if (!forcedly_pruned_edge_size && !selectively_pruned_edge_size) {
    KNOWHERE_THROW_MSG(
        "a lack of parameters forcedly_pruned_edge_size and selectively_pruned_edge_size 4 index "
        "NGTPANNG");
    return;
  }
  ...
  for (size_t id = 1; id < graph.repository.size(); id++) {
    try {
      ...
      std::cerr << "Graph::search: Warning. Cannot get the node. ID=" << id << ":" << err.what()
                << std::endl;
      continue;
    }
  }
...
void IndexNGTPANNG::UpdateIndexSize() {
  if (!index_) {

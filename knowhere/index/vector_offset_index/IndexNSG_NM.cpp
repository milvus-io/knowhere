...
BinarySet NSG_NM::Serialize(const Config& config) {
...
void NSG_NM::Load(const BinarySet& index_binary) {
  try {
...
DatasetPtr NSG_NM::Query(const DatasetPtr& dataset_ptr, const Config& config,
  ...
  try {
    ...
    index_->Search(reinterpret_cast<const float*>(p_data), reinterpret_cast<float*>(data_.get()),
                   rows, dim, topK, p_dist, p_id, s_params, bitset);
...
void NSG_NM::BuildAll(const DatasetPtr& dataset_ptr, const Config& config) {

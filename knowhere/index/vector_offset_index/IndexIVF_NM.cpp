...
BinarySet IVF_NM::Serialize(const Config& config) {
...
void IVF_NM::Load(const BinarySet& binary_set) {
  ...
  for (size_t i = 0; i < invlists->nlist; i++) {
    ...
    for (size_t j = 0; j < list_size; j++) {
      memcpy(arranged_data + d * (curr_index + j), original_data + d * ails->ids[i][j],
             d * sizeof(float));
    }
...
void IVF_NM::Train(const DatasetPtr& dataset_ptr, const Config& config) {
...
void IVF_NM::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  if (!index_ || !index_->is_trained) {
    KNOWHERE_THROW_MSG("index not initialize or trained");
  }
...
DatasetPtr IVF_NM::Query(const DatasetPtr& dataset_ptr, const Config& config,
...
VecIndexPtr IVF_NM::CopyCpuToGpu(const int64_t device_id, const Config& config) {
#ifdef KNOWHERE_GPU_VERSION
  if (auto res = FaissGpuResourceMgr::GetInstance().GetRes(device_id)) {
    ResScope rs(res, device_id, false);
    auto gpu_index = faiss::gpu::index_cpu_to_gpu_without_codes(
        res->faiss_res.get(), device_id, index_.get(), static_cast<const uint8_t*>(ro_codes->data));
...
void IVF_NM::GenGraph(const float* data, const int64_t k, GraphType& graph, const Config& config) {
  int64_t K = k + 1;
  auto ntotal = Count();
  ...
  for (int i = 0; i < total_search_count; ++i) {
    auto b_size =
        (i == (total_search_count - 1)) && tail_batch_size != 0 ? tail_batch_size : batch_size;
...
std::shared_ptr<faiss::IVFSearchParameters> IVF_NM::GenParams(const Config& config) {
...
void IVF_NM::QueryImpl(int64_t n, const float* query, int64_t k, float* distances, int64_t* labels,
...
#endif
  auto ivf_stats = std::dynamic_pointer_cast<IVFStatistics>(stats);
  ivf_index->search_without_codes(n, reinterpret_cast<const float*>(query), data, prefix_sum,
                                  is_sq8, k, distances, labels, bitset);
#if 0

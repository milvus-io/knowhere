...
void GPUIVF_NM::Train(const DatasetPtr& dataset_ptr, const Config& config) {
  ...
  if (gpu_res != nullptr) {
    ...
    auto device_index = new faiss::gpu::GpuIndexIVFFlat(gpu_res->faiss_res.get(), dim, nlist,
...
void GPUIVF_NM::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
  auto spt = res_.lock();
  if (spt != nullptr) {
...
void GPUIVF_NM::Load(const BinarySet& binary_set) {
  // not supported
}

VecIndexPtr GPUIVF_NM::CopyGpuToCpu(const Config& config) {
...
VecIndexPtr GPUIVF_NM::CopyGpuToGpu(const int64_t device_id, const Config& config) {
  auto host_index = CopyGpuToCpu(config);
  return std::static_pointer_cast<IVF>(host_index)->CopyCpuToGpu(device_id, config);
}

BinarySet GPUIVF_NM::SerializeImpl(const IndexType& type) {
...
void GPUIVF_NM::QueryImpl(int64_t n, const float* data, int64_t k, float* distances,
  ...
  if (device_index) {
    ...
    for (int64_t i = 0; i < n; i += block_size) {
      ...
      device_index->search(search_size, data + i * dim, k, distances + i * k, labels + i * k,
                           bitset);
    }
  } else {

...
template <typename T>
const TargetBitmapPtr StructuredIndexSort<T>::In(const size_t n, const T* values) {
  if (!is_built_) {
    build();
  }
...
template <typename T>
const TargetBitmapPtr StructuredIndexSort<T>::NotIn(const size_t n, const T* values) {
  if (!is_built_) {
    build();
  }
  TargetBitmapPtr bitset = std::make_unique<TargetBitmap>(data_.size());
...
template <typename T>
const TargetBitmapPtr StructuredIndexSort<T>::Range(const T value, const OperatorType op) {
  if (!is_built_) {
    build();
  }
  TargetBitmapPtr bitset = std::make_unique<TargetBitmap>(data_.size());
...
template <typename T>
const TargetBitmapPtr StructuredIndexSort<T>::Range(T lower_bound_value, bool lb_inclusive,

...
class StructuredIndexFlat : public StructuredIndex<T> {
 public:
  ...
  const TargetBitmapPtr In(const size_t n, const T* values) override;

  const TargetBitmapPtr NotIn(const size_t n, const T* values) override;

  const TargetBitmapPtr Range(const T value, const OperatorType op) override;

  const TargetBitmapPtr Range(T lower_bound_value, bool lb_inclusive, T upper_bound_value,

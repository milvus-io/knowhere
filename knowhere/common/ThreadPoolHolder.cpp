...
ThreadPoolHolder::~ThreadPoolHolder() {
    LOG_KNOWHERE_INFO_ << "liliutest call ThreadPoolHolder destroy " << pool_.use_count();
}
...
ThreadPoolHolder::GetInstance() {
    static auto p = std::make_shared<ctpl::thread_pool>(std::thread::hardware_concurrency());
    return p;
}
}  // namespace knowhere

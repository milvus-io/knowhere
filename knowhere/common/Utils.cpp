...
void Slice(const std::string& prefix, const BinaryPtr& data_src, const int64_t& slice_len,
           BinarySet& binarySet, milvus::json& ret) {
  if (!data_src) {
    return;
  }
...
void Assemble(BinarySet& binarySet) {
  ...
  milvus::json meta_data = milvus::json::parse(
      std::string(reinterpret_cast<char*>(slice_meta->data.get()), slice_meta->size));
...
void Disassemble(const int64_t& slice_size_in_byte, BinarySet& binarySet) {
  milvus::json meta_info;
  auto slice_meta = binarySet.Erase(INDEX_FILE_SLICE_META);
  if (slice_meta != nullptr) {
    milvus::json last_meta_data = milvus::json::parse(
        std::string(reinterpret_cast<char*>(slice_meta->data.get()), slice_meta->size));
  ...
  if (!slice_key_list.empty()) {
    ...
    std::shared_ptr<uint8_t> meta_data(new uint8_t[meta_str.length() + 1]);

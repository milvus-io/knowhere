...
class KnowhereConfig {
 public:
  ...
  static std::string SetSimdType(const SimdType simd_type);
   ...
   */
  static void SetBlasThreshold(const int64_t use_blas_threshold);
   ...
   */
  static void SetEarlyStopThreshold(const double early_stop_threshold);
  ...
  static void SetClusteringType(const ClusteringType clustering_type);
   ...
   * init GPU Resource
   */
  static void InitGPUResource(const std::vector<int64_t>& gpu_ids);

  /**
   * free GPU Resource
   */
  static void FreeGPUResource();
#endif
};
